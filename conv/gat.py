...
from torch_geometric.nn.conv import HANConv,HeteroConv, GATConv, GATConv
    ...
    def __init__(self, data, config, out_channels=1, multi=True,):
        ...
        for i in range(self.num_layers):
            self.layers.append(HeteroConv({
                            k: GATConv((-1,-1), self.hidden_channels//self.heads, add_self_loops=True, heads=self.heads) for k in data.edge_index_dict.keys()}, aggr='mean'))
