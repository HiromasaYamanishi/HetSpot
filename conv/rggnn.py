...
from torch_geometric.nn.conv import HANConv,HeteroConv, GATConv, RGATConv, GatedGraphConv
...
class HeteroRGGNN(torch.nn.Module):
    def __init__(self, data, config, out_channels=1, multi=True,):
        ...
        for i in range(self.num_layers):
            self.layers.append(HeteroConv({
                            k: GatedGraphConv(-1, self.hidden_channels,) for k in data.edge_index_dict.keys()
                        }, aggr='mean'))
