...
class HGT(torch.nn.Module):
    def __init__(self, data, config, out_channels=1, multi=True,):
        ...
        for i in range(self.num_layers):
            self.layers.append(HGTConv(in_channels=-1, out_channels=self.hidden_channels, metadata=data.metadata(), heads=8))
...
if __name__=='__main__':
    ...
    model = HGT(data, num_layers=3,hidden_channels=128, out_channels=1,multi=True)
    data.to(device)
    model.to(device)
    print(model)
