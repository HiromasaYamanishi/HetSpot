...
class HeteroGCNConv(torch.nn.Module):
    def __init__(self, in_channels_dict, x_dict, edge_index_dict, out_channels, ReLU, config):
        super().__init__()
...
class HeteroGCN(torch.nn.Module):
    def __init__(self, data, config, out_channels=1,multi=True):
        ...
        self.layers.append(HeteroGCNConv(self.first_in_channels_dict, x_dict, edge_index_dict, self.hidden_channels, self.ReLU, config))

        for i in range(self.num_layers-1):
            self.layers.append(HeteroGCNConv(self.mid_in_channels_dict, x_dict, edge_index_dict, self.hidden_channels, self.ReLU, config))
